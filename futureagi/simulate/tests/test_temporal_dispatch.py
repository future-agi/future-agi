...
@pytest.mark.unit
def test_execute_always_dispatches_to_temporal():
...
@pytest.mark.unit
def test_cancel_by_execution_id_always_dispatches_to_temporal():
    organization = SimpleNamespace(id=uuid4())
...
@pytest.mark.unit
def test_cancel_by_run_test_id_dispatches_latest_execution_to_temporal():
    organization = SimpleNamespace(id=uuid4())
    run_test = SimpleNamespace(id=uuid4(), organization=organization, deleted=False)
    test_execution = MagicMock()
    test_execution.id = uuid4()
    ...
    execution_query.order_by.return_value.first.return_value = test_execution

    with (
        patch("simulate.views.run_test.TestExecutor") as legacy_executor,
        patch("simulate.views.run_test.get_object_or_404", return_value=run_test),
        patch.object(
            ExecutionModel.objects, "filter", return_value=execution_query
        ) as execution_filter,
    ):
        view = CancelExecutionView()
        with patch.object(
    ...
    assert response.status_code == status.HTTP_200_OK
    execution_filter.assert_called_once_with(run_test=run_test)
    execution_query.order_by.assert_called_once_with("-created_at")
    temporal_dispatch.assert_called_once_with(test_execution)
