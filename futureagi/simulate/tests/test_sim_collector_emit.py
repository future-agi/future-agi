"""Unit tests for the sim → fi-collector OTLP export (TH-5642).

Pins the conversion of build_sim_spans dicts into OTLP spans: deterministic ids
survive into the OTLP id space, parent links are preserved, nested attributes
are coerced to OTLP-legal values, and the collector auth/resource contract is
honored. No Django / no live collector — the exporter is mocked.
"""
...
def test_otlp_attributes_coerces_nested_and_drops_none():
    ...
    assert out["tags"] == ["a", "b"]
    # Nested dict is JSON-serialized (OTLP cannot carry a dict value).
