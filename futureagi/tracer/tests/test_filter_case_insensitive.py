...
class TestSystemMetricTextCaseInsensitive:
    ...
    def test_contains_uses_utf8_literal_search(self, col):
    ...
    @pytest.mark.parametrize("col", ["trace_name", "name", "model", "provider"])
    def test_not_contains_uses_negated_utf8_literal_search(self, col):
    ...
    @pytest.mark.parametrize("col", ["trace_name", "name", "model", "provider"])
    def test_starts_with_uses_utf8_literal_boundary(self, col):
        where, _ = _builder().translate([_sm(col, "starts_with", "Foo")])
        assert f"startsWith(lowerUTF8(toString({col}))" in where

    @pytest.mark.parametrize("col", ["trace_name", "name", "model", "provider"])
    def test_ends_with_uses_utf8_literal_boundary(self, col):
    ...
    def test_trace_mode_wraps_utf8_literal_search_in_subquery(self):
        ...
        # The literal search belongs inside that wrap.
...
class TestSpanAttributeTextCaseInsensitive:
    ...
    def test_contains_uses_utf8_literal_search(self):
