...
_COST_CRASH_GAP = (
    "cost is_null/is_not_null crash CH Code 72 (Float64 compared to empty-string)"
)
...
def _sm_number_leaves():
    ...
    crash = {"only_targets": ("spans", "voiceCalls"), "contract_gap": _COST_CRASH_GAP}
    leaves = [
        ...
        ("is_null", None, lambda r: r.cost is None, crash),
        ("is_not_null", None, lambda r: r.cost is not None, crash),
    ]
    return [
...
def _trace_aggregate_leaves():
    ...
    # restricts to root spans. is_null/is_not_null crash CH (Code 72).
    tr = {"only_targets": ("traces",)}
    crash = {"only_targets": ("traces",), "contract_gap": _COST_CRASH_GAP}
    ...
    leaves = [
        ...
        ("cost", "is_null", None, lambda r: root(r) and r.cost is None, crash),
        ("cost", "is_not_null", None, lambda r: root(r) and r.cost is not None, crash),
    ]
    return [
