...
def test_text_discovery_is_granule_only_and_seed_retains_all_values(
    workspace, op, kind
):
    """Discovery reads NO Map value, whatever its leaves are.

    It used to reach that shape only for an all-thin-text conjunction, and it
    bought the shape by dropping every witness and probing a fixed DAY at a
    time. Both halves are gone: every leaf's witness is carried as an
    ``indexHint``, which prunes granules and reads nothing, and the width is
    proposed as the whole request for the row budget to fit. The seed is
    untouched and still applies every value.
    """
    filters = [
...
def test_every_operator_type_and_source_gets_one_full_width_proposal(extra):
    """One proposal for every shape: the day ladder had no shape left to serve,
    because no shape makes this statement read a Map value any more."""
    filters = [
    ...
    for leaves in (filters, [*filters, text_filter("region", "equals", "east")]):
        ...
        start, end = target._bounded_request_window
        assert target.recommended_filter_population_time_discovery_windows() in (
            None,
            (end - start,),
        )
...
def test_native_sparse_latest_and_cross_property_pages(
    ...
    # The published page above is the exactness claim; these two pin the shape
    # that made the proof cheap. No probe reads a Map value, and no probe is
    # bounded by a fixed number of days any more - the interval it covers is
    # the one the row budget approved.
    assert probes and all(
        "attrs_" not in _outside_index_hints(sql) for sql, _ in probes
    )
    assert all(p["population_end_us"] > p["population_start_us"] for _, p in probes)
