...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "_get_static_metric_graph_data_pg",
        ),
        (
            monitor_graphs.get_percentage_change_metric_graph_data,
            "_get_percentage_change_metric_graph_data_pg",
        ),
    ],
)
def test_clickhouse_timeout_never_starts_postgres_fallback(reader, pg_reader):
    analytics = Mock()
    analytics.execute_ch_query.side_effect = ReadDeadlineExceeded("CH timed out")
    pg_fallback = Mock(return_value=[])
    ...
    with (
        ...
        patch.object(monitor_graphs, pg_reader, pg_fallback),
    ):
...
def test_non_timeout_clickhouse_failure_can_use_remaining_postgres_budget():
    analytics = Mock()
    analytics.execute_ch_query.side_effect = RuntimeError("temporary CH failure")
    pg_fallback = Mock(return_value=[{"timestamp": "old", "value": 3}])
    ...
    with (
        ...
        patch.object(
            monitor_graphs, "monitor_graph_postgres_budget", return_value=nullcontext()
        ) as pg_budget,
        patch.object(monitor_graphs, "_get_static_metric_graph_data_pg", pg_fallback),
    ):
        result = monitor_graphs.get_static_metric_graph_data(
            _monitor(), deadline=ShrinkingDeadline()
        )

    assert result == [{"timestamp": "old", "value": 3}]
    pg_budget.assert_called_once()
    pg_fallback.assert_called_once()


def test_expired_wall_after_clickhouse_failure_never_starts_postgres():
    analytics = Mock()
