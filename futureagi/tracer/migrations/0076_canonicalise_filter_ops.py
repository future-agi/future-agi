...
def _migrate_user_alert_monitor(apps, stats):
    ...
    for m in UserAlertMonitor.objects.iterator(chunk_size=500):
        ...
        if isinstance(f, dict):
            stats["rewritten"] += _rewrite_filter_list(f.get("span_attributes_filters"))
...
def forwards(apps, schema_editor):
    ...
    _migrate_eval_task(apps, stats)
    _migrate_saved_view(apps, stats)
    _migrate_user_alert_monitor(apps, stats)
    ...
    _migrate_flat_list_field(CustomEvalConfig, "filters", stats)

    print(f"[canonicalise_filter_ops] {stats['rewritten']} ops rewritten")
