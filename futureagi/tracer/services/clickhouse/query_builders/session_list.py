...
class SessionListQueryBuilder(BaseQueryBuilder):
    ...
    def _build_having_clauses(self) -> str:
        ...
        for f in self.filters:
            ...
            # Numeric aggregate columns.
            if filter_op in ("is_null", "is_not_null"):
                conditions.append(
                    f"{ch_col} IS NULL"
                    if filter_op == "is_null"
                    else f"{ch_col} IS NOT NULL"
                )
                continue
            # serializer rejects number in/not_in (400); kept for WHERE-side parity.
            if filter_op in ("in", "not_in"):
                conditions.append(_in_clause(ch_col, filter_op, filter_value))
                continue
            if filter_op in ("between", "not_between"):
                if not isinstance(filter_value, list) or len(filter_value) != 2:
                    conditions.append("0 = 1")
                    continue
                lo, hi = _next_param(), _next_param()
                self.params[lo] = filter_value[0]
                self.params[hi] = filter_value[1]
                sql_op = "BETWEEN" if filter_op == "between" else "NOT BETWEEN"
                conditions.append(f"{ch_col} {sql_op} %({lo})s AND %({hi})s")
                continue
