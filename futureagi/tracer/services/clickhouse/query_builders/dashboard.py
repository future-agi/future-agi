    ...
    def _build_subquery_filters(
        ...
        for f in filters:
            ...
            if f_type == "eval_metric":
                ...
                output_type = (
                    f.get("output_type")
                    or resolve_eval_filter_metadata(
                ...
                # optional hint.
                resolved_output_type = f.get(
                    "output_type"
                ) or resolve_annotation_label_output_type(
                    label_id, self.organization_id
                )
                if not resolved_output_type:
                    raise InvalidMetricCombinationError(
                        f"'{leaf_column}' is not a known annotation label, so "
                        "it cannot be used as a dashboard filter."
                    )
                output_type = str(resolved_output_type).lower()
