...
# for the wall it owns on the plain-text-filtered page.
...
class UsersListManager:
    ...
    def list_cursor_payload(
        ...
        *,
        ...
        the walk unbounded. A plain-text span-attribute filter never reaches
        that walk: ``walk_matching_activity_page`` serves it and owns its own
        wall and statement budget (the same ``USER_LIST_PAGE_WALL_MS``),
        so ``page_wall`` does not apply to it.
        """
        ...
        # above owns the filtered page and starts its own wall. Here the refill
        # walk after the first batch runs at the page wall; the first batch
