...
EXPANSION_PATH_STARTED_SIGNALS = {
    "agent": ("agent_has_agent", "agents"),
    "evals": ("eval_has_source", "eval_source_count"),
    "gateway": ("gateway_has_provider", "gateway_keys", "gateway_requests"),
    "observe": ("observe_project_exists", "observe_projects", "traces"),
    "prompt": ("prompt_templates",),
    "voice": ("voice_has_agent", "voice_agents"),
}
EXPANSION_PATH_COMPLETED_SIGNALS = {
    "agent": "agent_first_loop_completed",
    "evals": "eval_first_loop_completed",
    "gateway": "gateway_first_loop_completed",
    "observe": "first_loop_completed",
    "prompt": "prompt_first_loop_completed",
    "voice": "voice_first_loop_completed",
}
    ...
    for campaign in campaigns:
        ...
        if daily_quality_modes is not None:
            ...
            if daily_quality_mode not in daily_quality_modes:
                continue
        if campaign.get("route_strategy") == "cross_path_expansion":
            campaign = _resolved_cross_path_expansion_campaign(
                activation_state,
                campaign,
            )
            if not campaign:
...
def _target_url(activation_state, campaign):
    ...
    if strategy == "daily_quality":
        ...
        href = _available_route_href(route)
        return _url_with_campaign_params(href, campaign)
    if strategy == "cross_path_expansion":
        return _url_with_campaign_params(
            campaign.get("expansion_target_href"), campaign
        )
    return None


def _path_signal_present(signals, names):
    for name in names:
...
def _path_started_or_completed(activation_state, path):
    signals = activation_state.get("signals") or {}
    completed_signal = EXPANSION_PATH_COMPLETED_SIGNALS.get(path)
    if completed_signal and bool(signals.get(completed_signal)):
        return True
    started_signals = EXPANSION_PATH_STARTED_SIGNALS.get(path) or ()
    return _path_signal_present(signals, started_signals)


def _cross_path_expansion_target(activation_state, campaign):
    route_availability = activation_state.get("route_availability") or {}
    primary_path = activation_state.get("primary_path")
    for path in campaign.get("expansion_target_paths") or ():
        if path == primary_path or _path_started_or_completed(activation_state, path):
            continue
        path_config = configured_path(path)
        action_id = path_config.get("first_action_id")
        if not action_id:
            continue
        action = configured_action(action_id)
        completion_event = action.get("completion_event")
        route = route_availability.get(action.get("route_key")) or {}
        href = _available_route_href(route)
        if href and completion_event:
            return {
                "action_id": action_id,
                "href": href,
                "path": path,
                "route_key": action.get("route_key"),
                "success_event": completion_event,
            }
    return None


def _resolved_cross_path_expansion_campaign(activation_state, campaign):
    target = _cross_path_expansion_target(activation_state, campaign)
    if not target:
        return None
    return {
        **campaign,
        "target_action_id": target["action_id"],
        "target_success_event": target["success_event"],
        "expansion_target_href": target["href"],
        "expansion_target_path": target["path"],
        "expansion_target_route_key": target["route_key"],
    }
