...
CROSS_PATH_EXPANSION_TEMPLATE_KEYS = frozenset({"cross_path_expansion_v1"})
DORMANT_REACTIVATION_TEMPLATE_KEYS = frozenset({"dormant_reactivation_v1"})
FIRST_LOOP_COMPLETE_TEMPLATE_KEYS = frozenset({"first_loop_complete_next_v1"})
...
def required_context_keys_for_template(template_key):
    ...
    if template_key in DIGEST_PREVIEW_TEMPLATE_KEYS:
        ...
        keys.add("digest_summary")
    if template_key in CROSS_PATH_EXPANSION_TEMPLATE_KEYS:
        keys.add("expansion_summary")
    if template_key in DORMANT_REACTIVATION_TEMPLATE_KEYS:
        keys.add("dormant_summary")
    if template_key in FIRST_LOOP_COMPLETE_TEMPLATE_KEYS:
        keys.add("first_loop_summary")
