...
def _real_execution_filter():
    return Q(execution_metadata__is_sample__isnull=True) | Q(
        execution_metadata__is_sample=False
    )
...
def _simulation_evidence(*, organization, workspace):
    ...
    if terminal_execution:
        call_execution = (
            terminal_execution.calls.filter(_real_call_filter())
            .filter(status=CallExecution.CallStatus.COMPLETED)
            .order_by("-completed_at", "-updated_at", "-created_at")
            .first()
            or terminal_execution.calls.filter(_real_call_filter())
            .order_by("-updated_at", "-created_at")
            .first()
        )
