...
class ActivationSignalsSerializer(serializers.Serializer):
    ...
    prompt_comparable_versions_exist = serializers.BooleanField(default=False)
    prompt_run_exists = serializers.BooleanField(default=False)
    prompt_committed_version_exists = serializers.BooleanField(default=False)
    prompt_comparison_completed = serializers.BooleanField(default=False)
    prompt_next_loop_action_exists = serializers.BooleanField(default=False)
    prompt_first_loop_completed = serializers.BooleanField(default=False)
    ...
    agent_has_step = serializers.BooleanField(default=False)
    ...
    agent_permission_limited = serializers.BooleanField(default=False)
...
class ActivationAgentStateSerializer(serializers.Serializer):
    ...
    has_step = serializers.BooleanField()
...
class ActivationValueMetricSerializer(serializers.Serializer):
    label = serializers.CharField()
    value = serializers.CharField()


class ActivationValueSignalSerializer(serializers.Serializer):
    """Privacy-safe proof for the activated home's "Latest proof" card.

    Observe can provide real trace metrics from ClickHouse. Other paths can
    provide generic headline/summary/metric evidence without raw prompts,
    traces, outputs, transcripts, or customer payloads.
    """
    ...
    metrics = ActivationValueMetricSerializer(many=True, required=False)
    ...
    def validate(self, attrs):
        has_trace_metrics = all(
            key in attrs for key in ("latency_ms", "cost", "total_tokens")
        )
        has_generic_proof = bool(
            attrs.get("summary") or attrs.get("headline") or attrs.get("metrics")
        )
        if not has_trace_metrics and not has_generic_proof:
            raise serializers.ValidationError(
                "Value signal requires trace metrics or generic proof."
            )
        return attrs
...
class ActivationStateResponseSerializer(serializers.Serializer):
    ...
    activated_via = serializers.ChoiceField(
        choices=["workspace", "organization"],
        required=False,
        allow_null=True,
    )
    ...
    value_signal = ActivationValueSignalSerializer(required=False, allow_null=True)
...
class TestTraceRequestSerializer(serializers.Serializer):
    path = serializers.CharField(default="observe")
    ...
    def validate_path(self, value):
        canonical = canonical_path(value)
        if canonical != "observe":
            raise serializers.ValidationError("Only observe test traces are supported.")
        return canonical


class TestTraceResponseSerializer(serializers.Serializer):
...
class TestTraceApiResponseSerializer(serializers.Serializer):
    status = serializers.BooleanField(default=True)
    result = TestTraceResponseSerializer()
