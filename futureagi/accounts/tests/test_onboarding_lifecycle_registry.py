...
def test_cross_path_expansion_campaign_uses_configured_first_actions():
    campaign = next(
        campaign
        for campaign in lifecycle_campaigns()
    ...
    assert campaign["campaign_group"] == "next_loop"
    assert campaign["primary_path"] == "any"
    assert campaign["route_strategy"] == "cross_path_expansion"
    assert campaign["dry_run_flag"] == "onboarding_email_next_loop_enabled"
    assert campaign["daily_quality_modes"] == ["no_new_signal"]
    assert campaign["expansion_target_paths"] == [
        "prompt",
        "evals",
        "gateway",
        "agent",
...
def test_lifecycle_registry_rejects_unknown_expansion_target_path():
    config = _valid_lifecycle_config()
    campaign = _campaign(config, "cross_path_expansion")
    campaign["expansion_target_paths"] = ["prompt", "billing"]

    with pytest.raises(ImproperlyConfigured):
        _validate_config(config)


def test_lifecycle_registry_rejects_expansion_targets_on_other_strategies():
    config = _valid_lifecycle_config()
    campaign = _campaign(config, "prompt_create_first")
    campaign["expansion_target_paths"] = ["agent"]
