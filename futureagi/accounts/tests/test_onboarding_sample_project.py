...
def test_sample_project_manifest_is_config_driven():
    ...
    assert set(manifest["optional_paths"]) == {
        "prompt",
        "evals",
        "agent",
        "voice",
    ...
    assert (
        manifest["optional_paths"]["prompt"]["preview"]["starterAction"]["label"]
        == "Run sample prompt comparison"
    )
    assert manifest["optional_paths"]["gateway"]["route"].startswith(
        "/dashboard/gateway"
    )
...
def test_sample_project_create_is_idempotent_and_opens_trace_route(
    ...
    optional_paths = sample_project.artifact_refs["optional_paths"]
    assert set(optional_paths) == {"prompt", "evals", "agent", "voice", "gateway"}
    assert optional_paths["prompt"]["artifact_type"] == "prompt_comparison"
    assert optional_paths["prompt"]["preview"]["layout"] == "promptDiff"
    assert optional_paths["prompt"]["metadata"]["onboarding_path"] == "prompt"
    assert optional_paths["gateway"]["route"].startswith("/dashboard/gateway")
    assert sample_project.health["missing_optional_paths"] == []


@pytest.mark.django_db
def test_sample_project_ready_repairs_missing_optional_path_refs(
    organization,
    workspace,
    user,
):
    ...
    refs = dict(sample_project.artifact_refs)
    refs.pop("optional_paths")
    sample_project.artifact_refs = refs
    sample_project.health = {}
    sample_project.save(update_fields=["artifact_refs", "health", "updated_at"])
    ...
    assert set(state["artifact_refs"]["optional_paths"]) == {
        "prompt",
        "evals",
        "agent",
        "voice",
