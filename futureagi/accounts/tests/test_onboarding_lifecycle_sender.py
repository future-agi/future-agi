...
def test_dormant_reactivation_lifetime_cap_ignores_old_daily_quality_digest(
    organization,
    workspace,
    user,
):
    _allow_user(user)
    now = timezone.now()
    prior_send = _sent_send_log_for_campaign(
        user,
        organization,
        workspace,
        campaign_key="daily_quality_open_actions",
    ...
    OnboardingLifecycleSendLog.no_workspace_objects.filter(id=prior_send.id).update(
        created_at=prior_send.sent_at,
    )
    log = _eligible_log(
        user,
        organization,
...
def test_dormant_reactivation_lifetime_cap_suppresses_repeat_send(
    organization,
    workspace,
    user,
):
...
def test_daily_quality_digest_send_log_carries_safe_preview(
    ...
    assert helper.call_args.args[0] == "1 quality item is waiting"
    assert template_context["preheader_text"] == (
        "Open Daily Quality: Assign trace owner."
    )
    assert template_context["digest_summary"]["action_count"] == 1
