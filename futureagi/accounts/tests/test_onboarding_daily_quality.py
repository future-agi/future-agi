...
@pytest.mark.django_db
def test_daily_quality_scans_all_activated_paths_from_observe_home(
    organization,
    workspace,
    user,
):
    ...
    daily_quality = payload["daily_quality"]
    cards_by_path = {card["path"]: card for card in daily_quality["product_cards"]}
    assert payload["stage"] == "daily_review"
    assert payload["primary_path"] == "observe"
    assert daily_quality["mode"] == "new_signal"
    assert daily_quality["top_signal"]["path"] == "gateway"
    assert daily_quality["top_signal"]["type"] == "gateway_request_issue"
    assert daily_quality["top_signal"]["source_id"] == str(request_log.id)
    assert daily_quality["primary_action"]["path"] == "gateway"
    assert payload["recommended_action"]["analytics"]["target_path"] == "gateway"
    assert list(cards_by_path) == ["observe", "gateway"]
    assert cards_by_path["observe"]["status"] == "healthy"
    assert cards_by_path["gateway"]["status"] == "needs_review"
    assert daily_quality["weekly_review"]["unresolved_count"] == 1
...
@pytest.mark.django_db
def test_daily_quality_promotes_fresh_prompt_comparison_signal(
    organization,
    workspace,
    user,
):
    ...
    latest_comparison = record_event(
        user=user,
        organization=organization,
        workspace=workspace,
        event_name="prompt_comparison_completed",
    ...
    assert payload["stage"] == "daily_review"
    assert daily_quality["mode"] == "new_signal"
    assert daily_quality["top_signal"]["path"] == "prompt"
    assert daily_quality["top_signal"]["type"] == "prompt_quality_review"
    assert daily_quality["top_signal"]["source_id"] == str(template.id)
    assert daily_quality["top_signal"]["created_at"] == latest_comparison.occurred_at
    assert daily_quality["primary_action"]["id"] == "review_prompt_quality"
    assert daily_quality["primary_action"]["path"] == "prompt"
    assert daily_quality["primary_action"]["route"].endswith(
        "source=onboarding&onboarding=metrics&tab=Metrics"
    )
    assert payload["recommended_action"]["analytics"]["target_path"] == "prompt"
    ...
    record_event(
        ...
        product_path="prompt",
        metadata={
            "signal_id": daily_quality["top_signal"]["id"],
            "signal_type": daily_quality["top_signal"]["type"],
            "source_type": daily_quality["top_signal"]["source_type"],
            "source_id": daily_quality["top_signal"]["source_id"],
        },
