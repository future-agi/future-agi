...
def _route(href="/dashboard/workbench/new", is_available=True):
    return {"href": href, "is_available": is_available, "reason": None}


def _expansion_activation_state(
    *, primary_path="observe", signals=None, mode="no_new_signal"
):
    return {
        "stage": "daily_review",
        "primary_path": primary_path,
        "is_activated": True,
        "daily_quality": {"mode": mode},
        "signals": {
            "first_loop_completed": primary_path == "observe",
            "prompt_templates": 0,
            "prompt_first_loop_completed": False,
            "eval_has_source": False,
            "eval_source_count": 0,
            "eval_first_loop_completed": False,
            ...
            "voice_has_agent": False,
            "voice_agents": 0,
            "voice_first_loop_completed": False,
            "observe_project_exists": primary_path == "observe",
            "observe_projects": 1 if primary_path == "observe" else 0,
            "traces": 1 if primary_path == "observe" else 0,
            **(signals or {}),
        },
        "route_availability": {
            "daily_quality_home": _route("/dashboard/home?mode=daily-quality"),
            "prompt_create": _route("/dashboard/workbench/new"),
            "eval_create_data": _route("/dashboard/evaluations/create"),
            "gateway_provider": _route("/dashboard/gateway/providers"),
            "agent_create": _route("/dashboard/agents/create"),
            "voice_create_agent": _route(
                "/dashboard/simulate/agent-definitions/create-new-agent-definition"
            ),
            "observe_setup": _route("/dashboard/observe?setup=true"),
        },
        "last_meaningful_event": {"occurred_at": timezone.now() - timedelta(days=2)},
        "recommended_action": {"id": "review_daily_quality"},
        "permissions": {"can_write": True},
        "sample_project": {},
    }


def test_choose_lifecycle_campaign_targets_next_unused_product_path():
    now = timezone.now()

    campaign = choose_lifecycle_campaign(
        _expansion_activation_state(),
        started_at=now - timedelta(days=2),
        now=now,
    )

    assert campaign["campaign_key"] == "cross_path_expansion"
    assert campaign["target_action_id"] == "create_prompt"
    assert campaign["target_success_event"] == "prompt_created"
    assert campaign["expansion_target_path"] == "prompt"


def test_cross_path_expansion_skips_started_candidate_paths():
    now = timezone.now()

    campaign = choose_lifecycle_campaign(
        _expansion_activation_state(signals={"prompt_templates": 1}),
        started_at=now - timedelta(days=2),
        now=now,
    )

    assert campaign["campaign_key"] == "cross_path_expansion"
    assert campaign["target_action_id"] == "create_eval_dataset"
    assert campaign["target_success_event"] == "eval_dataset_created"
    assert campaign["expansion_target_path"] == "evals"


def test_daily_quality_open_action_still_wins_over_cross_path_expansion():
    now = timezone.now()

    campaign = choose_lifecycle_campaign(
        _expansion_activation_state(mode="open_action"),
        started_at=now - timedelta(days=2),
        now=now,
    )
...
def test_cross_path_expansion_falls_back_when_all_paths_started_or_unavailable():
    now = timezone.now()
    state = _expansion_activation_state(
        signals={
            "prompt_templates": 1,
            "eval_has_source": True,
            "gateway_has_provider": True,
            "agent_has_agent": True,
            "voice_has_agent": True,
        },
    )
    state["route_availability"]["observe_setup"] = _route(
        "/dashboard/observe?setup=true",
        is_available=False,
    )
...
def test_cross_path_expansion_decision_routes_to_selected_target(
    organization,
    workspace,
    user,
):
    now = timezone.now()
    activation_state = _expansion_activation_state()
    activation_state["last_meaningful_event"] = {
    ...
    assert decision.campaign["campaign_key"] == "cross_path_expansion"
    assert decision.campaign["target_action_id"] == "create_prompt"
    assert decision.campaign["target_success_event"] == "prompt_created"
    assert decision.target_url.startswith("/dashboard/workbench/new?")
    assert "campaign_key=cross_path_expansion" in decision.target_url
    assert "target_event=prompt_created" in decision.target_url
