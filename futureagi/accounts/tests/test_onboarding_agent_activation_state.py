...
def test_graph_agent_without_step_returns_add_agent_node(organization, workspace, user):
    ...
    assert payload["signals"]["agent_has_step"] is False
...
def test_graph_agent_with_step_without_run_returns_run_scenario(
    ...
    assert payload["signals"]["agent_has_step"] is True
...
def test_agent_eval_event_activates_graph_path(organization, workspace, user):
    ...
    assert payload["value_signal"] == {
        ...
        "summary": "1 run reviewed · 1 eval ready",
        "metrics": [
            {"label": "Runs reviewed", "value": "1"},
            {"label": "Scenarios covered", "value": "1"},
            {"label": "Evals ready", "value": "1"},
        ],
    }
...
@pytest.mark.django_db
def test_agent_permission_limited_signal_is_exposed(organization, workspace, user):
    limited = _agent_state(
        user,
        organization,
        workspace,
        signals=OnboardingSignals(first_checks={}, agent_permission_limited=True),
    )
    assert limited["signals"]["agent_permission_limited"] is True

    default = _agent_state(
        user,
        organization,
        workspace,
        signals=OnboardingSignals(first_checks={}),
    )
    assert default["signals"]["agent_permission_limited"] is False


@pytest.mark.django_db
def test_sample_only_simulation_execution_does_not_advance_agent_stage(
    organization,
    workspace,
    user,
):
    ...
    execution = create_test_execution(run_test=run_test)
    execution.execution_metadata = {"is_sample": True}
    ...
    # A sample-only simulation must not count as a real run or advance the real
    # activation stage (mirrors the voice path's sample isolation).
    assert payload["agent"]["has_run"] is False
    assert payload["stage"] != "review_agent_trace"
