...
def test_activation_state_skips_first_run_when_org_has_product_setup(
    ...
    # Activation here is inherited from organization-level setup, not this
    # member's own completed loop.
    assert payload["activated_via"] == "organization"
...
def test_trace_review_without_improvement_returns_create_evaluator(
    ...
    assert payload["recommended_action"]["href"] == (
        ...
        "source=onboarding&step=data&source_type=trace_project&"
        f"source_id={project.id}&trace_id={payload['signals']['first_trace_id']}"
    )
    assert payload["fallback_action"]["id"] == "open_observe_dashboard_fallback"
    assert payload["fallback_action"]["href"] == f"/dashboard/observe/{project.id}"
...
def test_evaluator_after_trace_review_activates(organization, workspace, user):
    ...
    assert payload["stage"] == "activated"
    assert payload["is_activated"] is True
    # Activation earned by this workspace's own completed loop.
    assert payload["activated_via"] == "workspace"
...
@pytest.mark.django_db
def test_activated_observe_payload_exposes_value_signal_from_trace_review(
    organization,
    workspace,
    user,
):
    ...
    create_trace(project=project)
    create_custom_eval(organization=organization, workspace=workspace, project=project)
    # The real trace_reviewed event carries the observed value signal stamped by
    # the producer. Construct it directly with metadata={"signal": {...}} — no
    # ClickHouse needed in the consumer path.
    OnboardingActivationEvent.no_workspace_objects.create(
        ...
        metadata={
            ...
            "signal": {"latency_ms": 1200, "cost": 0.03, "total_tokens": 1240},
        },
    )
    signals = collect_onboarding_signals(
        user=user,
...
@pytest.mark.django_db
def test_activated_observe_payload_value_signal_none_without_trace_review(
    organization,
    workspace,
    user,
):
    ...
    create_trace(project=project)
    create_custom_eval(organization=organization, workspace=workspace, project=project)
    # A trace_reviewed event with no stamped signal must not invent one.
    record_event(
        user=user,
        organization=organization,
        workspace=workspace,
    ...
    assert payload["stage"] == "activated"
    assert payload["value_signal"] is None
