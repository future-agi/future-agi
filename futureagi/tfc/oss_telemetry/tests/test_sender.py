...
from accounts.utils import _fire_oss_telemetry_registration
from tfc.oss_telemetry.models import OSSTelemetryState
from tfc.oss_telemetry.payloads import build_full_registration_payload
from tfc.oss_telemetry.sender import (
    _post_payload,
...
def test_schedule_disables_temporal_retries():
    ...
    assert schedule.activity_name == "send_oss_telemetry_heartbeat"
...
def test_http_sender_retries_three_times(monkeypatch):
    ...
    with (
        patch("tfc.oss_telemetry.sender.is_oss", return_value=True),
        patch(
            "tfc.oss_telemetry.sender.requests.post",
            return_value=response,
        ) as post,
        patch("tfc.oss_telemetry.sender.time.sleep"),
    ):
        assert _post_payload("/oss/register", {"instance_id": "id"}) is False
...
def test_enabled_registration_sends_users_and_persists_full_state(monkeypatch):
    ...
    with (
        patch("tfc.oss_telemetry.sender.is_oss", return_value=True),
        patch(
            "tfc.oss_telemetry.sender._post_payload",
            return_value=True,
        ) as post,
    ):
        assert ensure_registration()[0] is True

    state = OSSTelemetryState.objects.get(singleton_key=1)
    assert state.registration_kind == OSSTelemetryState.RegistrationKind.FULL
    assert state.registration_status == OSSTelemetryState.RegistrationStatus.IDLE
    assert state.registration_metadata["users"] == [
        {"email": "admin@example.com", "domain": "example.com"}
    ]
    assert post.call_args.args[0] == "/oss/register"
...
def test_enabled_without_users_releases_registration_claim(monkeypatch):
    ...
    with (
        patch("tfc.oss_telemetry.sender.is_oss", return_value=True),
        patch("tfc.oss_telemetry.sender._post_payload") as post,
    ):
...
def test_registration_transitions_from_full_to_disabled(monkeypatch):
    ...
    with (
        patch("tfc.oss_telemetry.sender.is_oss", return_value=True),
        patch("tfc.oss_telemetry.sender._post_payload", return_value=True),
    ):
        assert ensure_registration()[0] is True
...
def test_successful_heartbeat_updates_window_state(monkeypatch):
    ...
    state.registration_kind = OSSTelemetryState.RegistrationKind.FULL
    state.save()
    ...
    with (
        patch("tfc.oss_telemetry.sender.is_oss", return_value=True),
        patch(
            "tfc.oss_telemetry.sender.compute_previous_utc_window",
            return_value=(window_start, window_end),
        ),
        patch(
            "tfc.oss_telemetry.sender.collect_counts",
            return_value=counts,
        ),
        patch(
            "tfc.oss_telemetry.sender._post_payload",
    ...
    assert post.call_args.args[0] == "/oss/heartbeat"
    state.refresh_from_db()
...
def test_cycle_failures_are_silent():
    with patch(
        "tfc.oss_telemetry.sender._run_telemetry_cycle",
