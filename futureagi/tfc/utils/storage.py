...
def _ssrf_safe_get(url, *, headers=None, timeout=20, max_bytes=None):
    """SSRF-guarded GET. Raises RequestException (not ValueError) so existing
    retry blocks in this module catch it like a normal connection failure.
    """
...
def download_document_from_url(doc_url, max_retries=5, timeout=20):
    ...
    for attempt in range(max_retries):
        try:
            response = _ssrf_safe_get(doc_url, headers=headers, timeout=timeout)
...
def download_image_from_url(image_url, max_retries=5, timeout=20):
    ...
    for attempt in range(max_retries):
        try:
            ...
            with _ssrf_safe_get(image_url, headers=headers, timeout=timeout) as response:
                logger.info(
...
def convert_image_from_url_to_base64(image_url, max_retries=5, timeout=120):
    for attempt in range(max_retries):
        try:
            response = _ssrf_safe_get(image_url, timeout=timeout)
