...
@pytest.mark.django_db
def test_simulation_byok_judge_skips_precheck_and_billing(
    monkeypatch, pass_fail_template, organization
):
    _patch_runner(monkeypatch, format_output_return="Passed")
    pass_fail_template.model = "gpt-4.1"
    pass_fail_template.save(update_fields=["model"])

    def unexpected(*_args, **_kwargs):
        raise AssertionError("BYOK simulation evaluator touched managed billing")

    monkeypatch.setattr(
        evals_module, "log_and_deduct_cost_for_api_request", unexpected
    )
    monkeypatch.setattr("ee.usage.services.metering.check_usage", unexpected)
    monkeypatch.setattr("ee.usage.services.emitter.emit", unexpected)
    ...
    output = run_eval_func(
        ...
        model="gpt-4.1",
        source="simulate",
        billing_event_id="41bfaad9-7f3c-5260-917b-67409ca4ca03",
    )
...
@pytest.mark.django_db
def test_simulation_managed_judge_emits_caller_stable_id(
    monkeypatch, pass_fail_template, organization
):
    _patch_runner(monkeypatch, format_output_return="Passed")
    event_id = "41bfaad9-7f3c-5260-917b-67409ca4ca03"
    checked = []
    emitted = []
    log_row = SimpleNamespace(
        log_id="log-managed",
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: (
            checked.append(True) or SimpleNamespace(allowed=True)
        ),
    )
    monkeypatch.setattr(
        "ee.usage.services.emitter.emit",
    ...
    run_eval_func(
        ...
        billing_test_execution_id="test-execution-1",
        billing_call_execution_id="call-execution-1",
    )

    assert checked == [True]
    assert len(emitted) == 1
    assert emitted[0].event_id == event_id
    assert emitted[0].properties["test_execution_id"] == "test-execution-1"
    assert emitted[0].properties["call_execution_id"] == "call-execution-1"


def test_simulation_code_evaluator_is_free_regardless_of_model():
    from model_hub.views.utils.evals import _simulation_evaluator_is_managed

    template = SimpleNamespace(
        eval_type="code",
        config={"eval_type_id": "CustomCodeEval"},
    )
    assert not _simulation_evaluator_is_managed(template, "turing_large")


def test_simulation_agent_judge_with_customer_model_is_free():
    from model_hub.views.utils.evals import _simulation_evaluator_is_managed
    ...
    assert not _simulation_evaluator_is_managed(template, "gpt-4.1")
    assert _simulation_evaluator_is_managed(template, "turing_large")
