    ...
    tools={
        ...
        "post": {
            ...
            "description": (
                ...
                "agent variants on a dataset. prompt_config entries: LLM "
                "prompt entries need prompt_id + prompt_version + model; "
                "agent entries need agent_id + agent_version; tts/stt/image "
                "entries need inline messages. user_eval_metrics (at least "
                "one) attaches evals: each entry is {template_id, name, "
                "config}. A snapshot of the dataset is taken and a Temporal "
                "workflow runs the experiment automatically."
            ),
        },
        "put": {
