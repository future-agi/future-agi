    ...
    async def acquire(self, resource_type: ResourceType, org_id: str, timeout: float = 30.0):
        ...
        try:
            async with asyncio.timeout(timeout):
                async with lock:
                    if not await self.can_acquire(resource_type, org_id):
                        self._rejected_requests += 1
                        quota = self.get_quota(org_id)
                        usage = self.get_usage(org_id)
                    ...
                    try:
                        ...
                        logger.info(
                            ...
                            org_id=org_id,
                            current_usage=self._get_usage_for_type(self.get_usage(org_id), resource_type)
                        )
