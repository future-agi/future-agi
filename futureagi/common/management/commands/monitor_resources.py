...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--org-id", type=str, help="Show detailed usage for specific organization"
        )
        parser.add_argument(
            "--detailed",
            action="store_true",
    ...
    async def _watch_mode(self, options):
        ...
        self.stdout.write(
            self.style.SUCCESS("Starting resource monitoring (Ctrl+C to stop)...")
        )
        ...
        try:
            while True:
                ...
                self.stdout.write(
                    f"Resource Monitor - {timezone.now().strftime('%Y-%m-%d %H:%M:%S')}"
                )
    ...
    def _print_org_details(self, metrics, org_id):
        ...
        resources = [
            ...
            (
                ...
                usage["optimization_workers"],
                quota.max_optimization_workers,
            ),
            (
                "Evaluation Workers",
                usage["evaluation_workers"],
                quota.max_evaluation_workers,
            ),
            (
                "LLM Requests/min",
                usage["llm_requests_last_minute"],
                quota.max_llm_requests_per_minute,
            ),
