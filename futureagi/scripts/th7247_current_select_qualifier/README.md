# TH-7247 current-source SELECT-only qualifier
...
[interactive read matrix](../../../docs/TH7247_INTERACTIVE_READ_MATRIX.md#exact-live-qualifier-boundary).
...
```sh
python3 futureagi/scripts/th7247_current_select_qualifier/assemble.py \
  --repo . \
  --output /tmp/fresh-th7247-bundle \
...
`deploy/dev/th7247-select-qualifier`: the Kubernetes path validates an exact
...
`th7247_catalog_dev_kartik_0816d`, without altering an existing application
...
PYTHONDONTWRITEBYTECODE=1 python3 \
  futureagi/scripts/th7247_current_select_qualifier/test_offline.py
...
```sh
PYTHONDONTWRITEBYTECODE=1 python3 \
  futureagi/scripts/th7247_current_select_qualifier/openapi_inventory.py
```
