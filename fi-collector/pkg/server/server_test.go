...
func TestServerEnd2End(t *testing.T) {
	...
	chSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		b := make([]byte, 1<<14)
		n, _ := r.Body.Read(b)
		// The collector now writes BOTH the pinned `spans` table and (for root
		// spans) the best-effort `traces` table. Scope this assertion to the
		// spans insert so the span-contract checks below stay exact; the traces
		// insert has its own test (TestServerEnd2End_WritesTraceRow).
...
// insertTable extracts the target table from a ClickHouse HTTP insert request's
// `query=INSERT INTO <table> FORMAT JSONEachRow` param (chwriter builds the URL
// this way). Lets a stub CH distinguish the pinned `spans` insert from the
// best-effort curated / `traces` inserts.
...
// TestServerEnd2End_WritesTraceRow proves the new-connection fix end-to-end at
// the server boundary: a single OTLP root span flushed through the real
// converter + curatedwriter must produce a `traces` insert (so trace_dict
// resolves the trace's project_id / name for evals & annotations), in ADDITION
// to the spans insert.
