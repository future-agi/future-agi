...
func TestBatch_TraceDedupWithinBatch(t *testing.T) {
	b := NewBatch()
	// One trace can have many spans, but only the root span calls AddTrace; even
	// if a buggy producer emits two roots, the batch must collapse to one row.
...
func TestWrite_Traces_ColumnMapping(t *testing.T) {
	...
	// _version is a UInt64 → JSONEachRow number → decoded float64; must be the
	// root span's start nanos (so the app mirror's later time.time_ns wins).
	if f, _ := row["_version"].(float64); f != 1700000000000000000 {
		t.Errorf("traces._version: got %#v want 1700000000000000000", row["_version"])
	}
}

// TestWrite_Traces_NameSessionOmittedWhenEmpty proves the ~96%-unnamed /
// no-session case: an empty Name / SessionID must be OMITTED from the row so the
// Nullable(String)/Nullable(UUID) columns land as SQL NULL (the dict's 'name'
// round-trips NULL), not as an empty string that would shadow it.
