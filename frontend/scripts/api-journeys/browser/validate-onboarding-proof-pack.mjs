...
const REQUIRED_FIRST_VALUE_PROPERTIES = [
  "source",
  "quick_start_goal",
  "quick_start_id",
...
function isFirstValueContractValid(event, { dailyQualityAvailable } = {}) {
  const properties = event?.properties || {};
  return (
    event?.event === "onboarding_first_value_reached" &&
...
function launchMetricReportEntry(child, expected, report) {
  ...
  const firstValueProperties =
    evidence.first_value_posthog_event?.properties || {};
  const uiFirstValueContractValid = isFirstValueContractValid(
    evidence.first_value_posthog_event,
    {
  ...
  return {
    ...
    ui_first_value_event:
      evidence.first_value_posthog_event?.event ===
      "onboarding_first_value_reached",
    ui_first_value_contract_valid: uiFirstValueContractValid,
    ui_first_value_required_properties_missing: missingProperties(
      evidence.first_value_posthog_event,
      REQUIRED_FIRST_VALUE_PROPERTIES,
    ),
    ui_first_value_daily_quality_available:
      firstValueProperties.daily_quality_available === true,
    ui_first_value_is_sample: firstValueProperties.is_sample === true,
    ui_first_value_quick_start_id: firstValueProperties.quick_start_id || null,
    ui_first_value_quick_start_primary_path:
      firstValueProperties.quick_start_primary_path || null,
    ui_first_value_activation_event_name:
      firstValueProperties.activation_event_name || null,
    ui_first_value_activation_event_path:
      firstValueProperties.activation_event_path || null,
  };
}
...
function buildLaunchMetrics(entries, checks) {
  ...
  const sampleEntries = entries.filter(
    (entry) => entry.proof === "sample_guard",
  );
  const backendLoopCount = realEntries.filter(
    (entry) => entry.backend_loop_completed,
  ).length;
  const uiFirstValueEntries = realEntries.filter(
    (entry) => entry.ui_first_value_contract_valid,
  );
  const uiFirstValueRequiredPropertyFailures = realEntries.filter(
    (entry) =>
      entry.ui_first_value_event &&
      entry.ui_first_value_required_properties_missing.length > 0,
  );
  const failedChecks = checks.filter((check) => !check.passed);

  return {
    schema_version: LAUNCH_METRICS_SCHEMA,
    dashboard_contract: "signup_to_first_value_posthog_checks",
    children: {
    ...
    ui_first_value: {
      ...
      ui_first_value_count: uiFirstValueEntries.length,
      backend_to_ui_first_value_rate: ratio(
        uiFirstValueEntries.length,
        backendLoopCount,
      ),
      daily_quality_available_count: uiFirstValueEntries.filter(
        (entry) => entry.ui_first_value_daily_quality_available,
      ).length,
      required_property_failure_count:
        uiFirstValueRequiredPropertyFailures.length,
      quick_start_ids: uniqueSorted(
        uiFirstValueEntries.map((entry) => entry.ui_first_value_quick_start_id),
      ),
      quick_start_primary_paths: uniqueSorted(
        uiFirstValueEntries.map(
          (entry) => entry.ui_first_value_quick_start_primary_path,
        ),
      ),
      activation_event_names: uniqueSorted(
        uiFirstValueEntries.map(
          (entry) => entry.ui_first_value_activation_event_name,
        ),
      ),
      activation_event_paths: uniqueSorted(
        uiFirstValueEntries.map(
          (entry) => entry.ui_first_value_activation_event_path,
        ),
      ),
    },
    sample: {
    ...
    guardrails: {
      ...
      missing_ui_first_value_count: Math.max(
        backendLoopCount - uiFirstValueEntries.length,
        0,
      ),
    },
  };
...
function validateSampleGuardEvidence(checks, childId, report) {
  const evidence = report.evidence || {};
  validateRealQualityLoopEvidence(checks, childId, report);
  addCheck(
    checks,
    evidence.setup_quick_start === "observe",
    `${childId}:sample_guard:quick_start`,
    "Sample guard continues through the real Observe quick start.",
  );
  addCheck(
    checks,
    isSetupOrgHomeUrl(evidence.setup_org_home_url) &&
      hasExpectedParams(evidence.setup_org_home_url, {
        source: "setup_org",
        ...OBSERVE_QUICK_START_ATTRIBUTION,
      }),
    `${childId}:sample_guard:setup_home_attribution`,
    "Sample guard keeps setup-org Home attribution before the first real action.",
  );
  addCheck(
    checks,
    evidence.sample_project_post === null,
    `${childId}:sample_guard:no_sample_project_post`,
    "Sample guard does not open or create the sample project before real setup.",
  );
  addCheck(
    checks,
    evidence.sample_trace_activation_event === null,
    `${childId}:sample_guard:no_sample_trace_event`,
    "Sample guard does not record sample trace review before real setup.",
  );
  addCheck(
    checks,
    evidence.sample_to_real_setup_event === null,
    `${childId}:sample_guard:no_sample_to_real_handoff`,
    "Sample guard does not require a sample-to-real handoff before real setup.",
  );
}
...
function validateRealQualityLoopEvidence(checks, childId, report) {
  ...
  addCheck(
    ...
    `${childId}:real_loop:setup_home_attribution`,
    "Real proof keeps setup-org Home attribution before the first action.",
  );
  addCheck(
    checks,
  ...
  addCheck(
    checks,
    isFirstValueContractValid(evidence.first_value_posthog_event, {
      dailyQualityAvailable: shouldHaveDailyQuality,
    }),
    `${childId}:real_loop:first_value_posthog`,
    "Real proof captures the frontend first-value PostHog marker with observe quick-start attribution.",
  );
  addCheck(
    checks,
    Boolean(
...
function validateReport(checks, manifest, child, expected, report) {
  ...
  } else if (expected.proof === "sample_guard") {
    validateSampleGuardEvidence(checks, expected.id, report);
  } else {
...
function textOutput(result) {
  const lines = [
    ...
    `ui_first_value=${result.launch_metrics.ui_first_value.ui_first_value_count}/${result.launch_metrics.ui_first_value.backend_loop_completed_count}`,
    `backend_to_ui_first_value_rate=${result.launch_metrics.ui_first_value.backend_to_ui_first_value_rate}`,
