...
async function main() {
  ...
  try {
    if (START_FROM_HOME) {
      ...
      await waitForVisibleText(page, "Prove a prompt edit is better", {
        exact: true,
      });
      await waitForVisibleText(
        page,
        "Test a prompt on real cases, lock a baseline, and see which edit wins. Current step: Write the prompt you want to improve. After it is done, Home will show See how it scores on real cases.",
      );
      await waitForVisibleText(page, "Current step", { exact: true });
      await waitForVisibleText(page, "Step 1 of 6", { exact: true });
      await waitForVisibleText(page, "Write the prompt you want to improve", {
        exact: true,
      });
      await waitForVisibleText(page, "Later steps", { exact: true });
      await waitForVisibleText(page, "See how it scores on real cases", {
        exact: true,
      });
    ...
    await waitForVisibleText(page, "Create prompt", { exact: true });
    if (
      !(await visibleTextExists(page, "Create a prompt to test", {
        exact: true,
      }))
    ) {
      await clickVisibleText(page, "Create prompt", { exact: true });
    }
    await waitForVisibleText(page, "Create a prompt to test", { exact: true });
    ...
    await waitForSearchParam(page, "onboarding", "run-test");
    await waitForVisibleText(page, "Run the second version", { exact: true });
    await dismissTourIfPresent(page);
    ...
    // The second-version run step keeps the create_second_prompt_version journey
    // anchor, so the run action is exposed via prompt_create_second_version_button.
    await clickSelector(
      page,
      '[data-tour-anchor="prompt_create_second_version_button"]',
    );
    await waitForSearchParam(page, "onboarding", "save-version");
    await waitForVisibleText(page, "Save the second version", {
      exact: true,
    });
    ...
    await commitCurrentOnboardingVersion(page, {
      ...
      // The second-version save step keeps the create_second_prompt_version
      // journey anchor rather than the first-version save anchor.
      saveAnchor: "prompt_create_second_version_button",
    });
    ...
    await clickVisibleButtonText(page, "Finish setup", {
      exact: true,
...
async function commitCurrentOnboardingVersion(
  page,
  { message, saveAnchor = "prompt_save_version_button" },
) {
  await clickSelector(page, `[data-tour-anchor="${saveAnchor}"]`);
