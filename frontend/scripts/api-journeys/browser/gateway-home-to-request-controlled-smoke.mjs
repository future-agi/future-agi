...
async function main() {
  ...
  try {
    ...
    await waitForVisibleText(page, "Route LLM traffic safely", {
      exact: true,
    });
    await waitForVisibleText(
      page,
      "Route your first request, see cost and latency per call, then add a guardrail. Current step: See cost + latency per call. After it is done, Home will show Trace where time and spend went.",
    );
    await waitForVisibleText(page, "Current step", { exact: true });
    await waitForVisibleText(page, "Step 3 of 6", { exact: true });
    await waitForVisibleText(page, "See cost + latency per call", {
      exact: true,
    });
    await waitForVisibleText(page, "Send request", { exact: true });
    await waitForVisibleText(page, "Later steps", { exact: true });
    await waitForVisibleText(page, "Trace where time and spend went", {
      exact: true,
    });
