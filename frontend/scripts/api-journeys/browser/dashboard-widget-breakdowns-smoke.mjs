...
const SCREENSHOT_PATH = "/tmp/dashboard-widget-breakdowns-smoke.png";
const FAILURE_SCREENSHOT_PATH =
  "/tmp/dashboard-widget-breakdowns-smoke-failure.png";

async function main() {
  const auth = await createAuthenticatedContext();
  const runSuffix = auth.runId;
  const dashboardName = `TH-4812 widget breakdowns ${runSuffix}`;
  const widgetName = `TH-4812 breakdown widget ${runSuffix}`;
  ...
  const breakdownInventory = await auth.client.get(
    apiPath("/tracer/dashboard/metrics/"),
    {
      query: {
        category: "system_metric",
  ...
  assert(
    asArray(breakdownInventory?.metrics || breakdownInventory).some(
      (metric) => metric?.name === "project",
    ),
    "Dashboard metric catalog did not include project breakdown attribute.",
  );

  const dashboard = await auth.client.post(apiPath("/tracer/dashboard/"), {
    name: dashboardName,
    description:
      "Disposable dashboard for widget chart type and breakdowns smoke.",
  });
  ...
  try {
    ...
    await selectChartType(page, "Stacked Column");
    ...
    await clickSectionTitle(page, "Breakdown");
    await fillPickerSearch(
      page,
      "Search breakdown attributes",
      "project",
      "Project",
    );
    await clickVisibleText(page, "Project", { exact: true });
      ...
      () =>
        queryEvents.some(
          (event) =>
            ...
            hasBreakdown(event.payload, "project"),
        ),
      "successful preview query with latency metric and project breakdown",
    );
    ...
    assert(
      ...
      `Dashboard breakdown API failures: ${apiFailures.join("; ")}`,
    );
...
function assertWidgetShape(widget) {
  ...
  assert(
    (chartConfig.chart_type || chartConfig.chartType) === "stacked_column",
    "Widget chart_config did not persist stacked_column.",
  );
  assert(
  ...
  assert(
    asArray(queryConfig.breakdowns).some(
      (breakdown) => breakdown.name === "project" || breakdown.id === "project",
    ),
    "Widget query_config did not include project breakdown.",
  );
}
...
async function selectChartType(page, label) {
  const opened = await page.evaluate(() => {
    ...
    const chartSelect = selects.find(
      (element) => isVisible(element) && element.textContent.includes("Line"),
    );
    if (!chartSelect) return false;
    chartSelect.dispatchEvent(
  ...
  assert(opened, "Unable to open chart type selector.");
...
SELECT json_build_object(
  ...
  'breakdown_names', (
    SELECT json_agg(breakdown ->> 'name')
    FROM widget_row, jsonb_array_elements(query_config -> 'breakdowns') breakdown
  ),
...
function assertWidgetConfigAudit(
  ...
  assert(
    audit?.chart_type === "stacked_column",
    "Audit did not find stacked_column chart type.",
  );
  assert(
  ...
  assert(
    asArray(audit?.breakdown_names).includes("project"),
    "Audit did not find project breakdown.",
  );
...
function hasBreakdown(payload, breakdownName) {
  return asArray(payload?.breakdowns).some(
    (breakdown) =>
      breakdown.name === breakdownName || breakdown.id === breakdownName,
  );
}
