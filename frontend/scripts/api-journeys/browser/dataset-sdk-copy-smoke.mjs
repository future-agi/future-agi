...
const SDK_CODE_COPY_ARIA_LABEL = "Copy";
...
async function main() {
  ...
  const cleanupDatasetIds = [];
  let browser = null;
  let page = null;

  try {
    const source = await selectDatasetForSdkCopy(auth.client, auth.runId);
    const { datasetId, datasetName, tableColumnName } = source;
    if (source.created) cleanupDatasetIds.push(datasetId);
    ...
    await waitForVisibleText(page, tableColumnName, { exact: true });
    ...
    snippetCopies.python = await clickAndReadClipboard(page, {
      ariaLabel: SDK_CODE_COPY_ARIA_LABEL,
      scopeText: SDK_MODAL_HEADING,
    });
    ...
    if (pageErrors.length) {
      console.error(`page_errors=${JSON.stringify(pageErrors.slice(-10))}`);
    }
    if (apiFailures.length) {
      console.error(`api_failures=${JSON.stringify(apiFailures.slice(-10))}`);
    }
    if (failedRequests.length) {
      console.error(
        `failed_requests=${JSON.stringify(failedRequests.slice(-10))}`,
      );
    }
    if (consoleMessages.length) {
      console.error(
        `browser_console=${JSON.stringify(consoleMessages.slice(-20))}`,
      );
    }
    throw error;
  } finally {
    if (cleanupDatasetIds.length) {
      await deleteDatasets(
        ...
        cleanupEvidence.push({
          cleanup: "delete dataset SDK copy fallback dataset after failure",
          status: "failed",
          error: error.message,
          dataset_ids: cleanupDatasetIds.slice(),
        });
      });
    }
...
async function selectDatasetForSdkCopy(client, runId) {
  ...
  for (const dataset of asArray(listPayload)) {
    if (!dataset?.id || !dataset?.name) continue;
    const table = await getDatasetTable(client, dataset.id).catch(() => null);
    const columns = asArray(table?.column_config);
    const firstColumn = columns.find((column) => column?.name);
    if (firstColumn) {
      return {
        datasetId: dataset.id,
        datasetName: dataset.name,
        tableColumnName: firstColumn.name,
        created: false,
      };
    }
  }

  return createSourceDataset(client, runId);
}

async function createSourceDataset(client, runId) {
  requireMutations();
  ...
  const firstColumn = asArray(table?.column_config).find(
    (column) => column?.name,
  );
  assert(firstColumn, "Dataset SDK copy fallback dataset had no columns.");
...
async function deleteDatasets(client, datasetIds, evidence) {
  ...
  evidence.push({
    cleanup: "delete dataset SDK copy fallback dataset",
    status: "passed",
