...
const EXPECT_DAILY_QUALITY =
  process.env.ONBOARDING_SMOKE_EXPECT_DAILY_QUALITY === undefined
    ? true
    : envFlag("ONBOARDING_SMOKE_EXPECT_DAILY_QUALITY");
const ALLOW_TRACE_CH_FAILURES = envFlag(
  "ONBOARDING_SMOKE_ALLOW_TRACE_CH_FAILURES",
);
const REPORT_OUTPUT = process.env.ONBOARDING_SMOKE_REPORT_OUTPUT || "";
const POSTHOG_SMOKE_KEY =
  process.env.ONBOARDING_SMOKE_POSTHOG_KEY || "phc_local_onboarding_smoke";
const POSTHOG_SMOKE_HOST =
  process.env.ONBOARDING_SMOKE_POSTHOG_HOST || "https://local.posthog.test";
...
async function main() {
  ...
  await page.evaluateOnNewDocument(
    ...
    {
      VITE_HOST_API: API_BASE,
      VITE_POSTHOG_KEY: POSTHOG_SMOKE_KEY,
      VITE_POSTHOG_HOST: POSTHOG_SMOKE_HOST,
    },
  );
  ...
  page.on("request", (request) => {
    ...
    if (isPostHogRequest(url)) {
      ...
      // Smoke mode disables PostHog compression so the capture body stays
      // plain application/json. That content type triggers a CORS preflight;
      // without these headers the browser aborts the preflight and the real
      // POST never fires, so the event payload can never be intercepted.
      const corsHeaders = {
        "Access-Control-Allow-Origin": requestHeaders.origin || appOrigin,
      ...
      // PostHog loads web-vitals.js and the array config.js via <script> tags.
      // Serving JSON for those makes the browser execute JSON as JavaScript
      // ("Unexpected token ':'" page errors), so answer script requests with a
      // valid empty script instead of the JSON status body.
  ...
  page.on("response", async (response) => {
    ...
    if (
      ...
      if (isAllowedTraceClickHouseFailure(failure)) {
        evidence.allowedApiFailures.push(failure);
      } else {
  ...
  try {
    ...
    await expectVisibleText(
      page,
      "What are you trying to improve first? We will open the first screen for it, then Home will keep showing the next step.",
      { timeout: 90000 },
    );
    if (SAMPLE_GATE_ONLY) {
    ...
    if (!SAMPLE_ONLY) {
      ...
      await expectVisibleText(
        page,
        "Use the OpenAI Python setup below, run one request, and keep this page open while we wait for the trace. After review, the next step is the first quality check.",
        { timeout: 45000 },
      );
      await expectVisibleText(page, "Connect OpenAI Python", {
        timeout: 45000,
    ...
    if (SAMPLE_ONLY) {
      ...
      const report = smokeReportPayload({
        ...
        evidence: {
          ...
          allowed_api_failures: evidence.allowedApiFailures,
    ...
    const firstValuePostHogEvent = postHogSmokeSkipped
      ? null
      : await waitForFirstValuePostHogEvent(evidence.posthogEvents, {
    ...
    const report = smokeReportPayload({
      ...
      evidence: {
        ...
        post_first_value_cta_href: postFirstValueCtaHref,
        expected_daily_quality_available: EXPECT_DAILY_QUALITY,
        posthog_smoke_skipped: postHogSmokeSkipped,
        first_value_posthog_event: firstValuePostHogEvent,
...
async function waitForFirstValuePostHogEvent(
  posthogEvents,
  { dailyQualityAvailable } = {},
) {
  let firstValuePostHogEvent = null;
  await waitForCondition(
    () => {
      firstValuePostHogEvent = posthogEvents.find((event) =>
        isFirstValuePostHogEvent(event, { dailyQualityAvailable }),
      );
      return Boolean(firstValuePostHogEvent);
    },
    "Expected frontend first-value PostHog event with observe quick-start attribution.",
    45000,
  );
  return firstValuePostHogEvent;
}

function isFirstValuePostHogEvent(event, { dailyQualityAvailable } = {}) {
...
function isAllowedTraceClickHouseFailure(failure) {
  if (!ALLOW_TRACE_CH_FAILURES) return false;
  return (
    failure === "400 /tracer/trace/list_traces_of_session/" ||
    /^400 \/tracer\/trace\/[^/]+\/$/.test(failure)
  );
}
