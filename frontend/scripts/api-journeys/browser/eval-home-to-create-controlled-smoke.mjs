...
async function main() {
  ...
  try {
    ...
    await waitForVisibleText(page, "Catch failing responses before users do", {
      exact: true,
    });
    await waitForVisibleText(
      page,
      "Pick what to test, see which examples pass and fail, and find out exactly why. Current step: Pick what to test. After it is done, Home will show Define what good looks like.",
    );
    await waitForVisibleText(page, "Current step", { exact: true });
    await waitForVisibleText(page, "Step 1 of 5", { exact: true });
    await waitForVisibleText(page, "Pick what to test", { exact: true });
    await waitForVisibleText(page, "Create dataset", { exact: true });
    await waitForVisibleText(page, "Later steps", { exact: true });
    await waitForVisibleText(page, "Define what good looks like", {
      exact: true,
    });
    ...
    await waitForVisibleText(page, "Eval setup", { exact: true });
    await waitForVisibleText(page, "Choose what to test", { exact: true });
    await waitForVisibleText(
      page,
