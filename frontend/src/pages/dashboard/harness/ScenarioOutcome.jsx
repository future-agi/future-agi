...
export default function ScenarioOutcome({ outcome }) {
  ...
  // failed checks nobody ran; leaving it only in the denominator claims the opposite, that
  // it passed. It gets its own count and its own line.
  const failed = outcome.subGoals.filter((goal) => goal.held === false);
  const unjudged = outcome.subGoals.filter((goal) => goal.held == null);
  const judged = outcome.subGoals.length - unjudged.length;
  const checksLabel = [
    judged > 0 ? `${failed.length} of ${judged} checks failed` : "",
    unjudged.length > 0 ? `${unjudged.length} not judged` : "",
  ]
    .filter(Boolean)
    .join(", ");
  ...
  return (
    <Stack spacing={0.75} sx={{ mt: 0.75 }}>
      <Stack direction="row" alignItems="center" spacing={1}>
        ...
        {(failed.length > 0 || unjudged.length > 0) && (
          <Link
            component="button"
            type="button"
            variant="caption"
      ...
      <Collapse in={open} unmountOnExit>
        <Stack component="ul" spacing={0.25} sx={{ m: 0, pl: 2 }}>
          {[...failed, ...unjudged].map((goal) => (
            <Box component="li" key={goal.name}>
              <Typography variant="caption" color="text.secondary">
                <Box component="span" sx={{ color: "text.primary" }}>
                  {goal.name}
                  {goal.held == null ? " (not judged)" : ""}
                </Box>
