...
describe("ScenarioOutcome", () => {
  ...
  it("keeps the failed-check reasons behind a toggle", async () => {
    ...
    expect(
      screen.queryByText(/book_ride never succeeded/),
    ).not.toBeInTheDocument();
    await user.click(
      screen.getByRole("button", { name: /1 of 2 checks failed/ }),
    );
  ...
  it("offers nothing to open when every goal held", () => {
    render(
      <ScenarioOutcome
        outcome={{
          ...
          subGoals: [
            {
              name: "suspension_refused_with_handoff",
              held: true,
              reason: null,
            },
          ],
    ...
    expect(
      screen.queryByRole("button", { name: /checks failed/ }),
    ).not.toBeInTheDocument();
  });
  ...
  it("renders nothing when the outcome carries nothing to show", () => {
    const { container } = render(
      <ScenarioOutcome
        outcome={{ status: null, turns: null, durationMs: null, subGoals: [] }}
      />,
    );
    expect(container).toBeEmptyDOMElement();
  });
...
describe("ScenarioOutcome — unjudged checks and failures", () => {
  const errored = {
    ...
    failure: {
      ...
      message:
        "target agent never joined the room: Target agent did not become ready",
    },
  ...
  it("shows the failure that actually explains the run", () => {
    ...
    expect(
      screen.getByText(/Environment · World unavailable/),
    ).toBeInTheDocument();
  });

  it("counts a real failed check and says the unjudged one was never judged", async () => {
    const user = userEvent.setup();
    render(
      <ScenarioOutcome
        outcome={{
          ...
          subGoals: [
            {
              name: "booking_status_checked",
              held: false,
              reason: "get_booking_status was never called",
            },
    ...
    const link = screen.getByRole("button", {
      name: /1 of 1 checks failed, 1 not judged/,
    });
    expect(link).toBeInTheDocument();
    await user.click(link);
    expect(screen.getByText(/unjudged_one \(not judged\)/)).toBeInTheDocument();
  });

  it("reports a call whose every check broke as judged by nobody", () => {
    render(
      <ScenarioOutcome
        outcome={{
          status: "failed",
          subGoals: [
            { name: "one", held: null, reason: "check raised" },
            { name: "two", held: null, reason: null },
          ],
        }}
      />,
    );
    expect(
      screen.getByRole("button", { name: /2 not judged/ }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /checks failed/ }),
    ).not.toBeInTheDocument();
