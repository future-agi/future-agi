...
describe("activation-state utilities", () => {
  it.each(activationStateFixtureList)(
    ...
    ({ state }) => {
      ...
      expect(normalized.schemaVersion).toBe("activation-state-2026-06-08.v2");
  ...
  it("normalizes prompt activated value proof without observe metrics", () => {
    const normalized = normalizeActivationState(
      getActivationStateFixture("promptActivated"),
    );

    expect(normalized.primaryPath).toBe("prompt");
    expect(normalized.valueSignal.kind).toBe("prompt_quality_loop");
    expect(normalized.valueSignal.summary).toBe(
      "2 versions compared · 1 quality check ready",
    );
    expect(normalized.valueSignal.metrics).toEqual([
      {
    ...
    expect(normalized.valueSignal.latencyMs).toBeNull();
    expect(normalized.valueSignal.cost).toBeNull();
    expect(normalized.valueSignal.totalTokens).toBeNull();
  });

  it("normalizes non-observe activated value proof without observe metrics", () => {
    const normalized = normalizeActivationState(
      getActivationStateFixture("gatewayPolicyCreated"),
    );

    expect(normalized.primaryPath).toBe("gateway");
    expect(normalized.valueSignal.kind).toBe("gateway_routing_loop");
    expect(normalized.valueSignal.summary).toBe(
      "1 request routed · 1 policy ready",
    );
    expect(normalized.valueSignal.metrics).toEqual([
      { label: "Requests routed", value: "1" },
      { label: "Policies ready", value: "1" },
    ]);
    expect(normalized.valueSignal.latencyMs).toBeNull();
    expect(normalized.valueSignal.cost).toBeNull();
    expect(normalized.valueSignal.totalTokens).toBeNull();
