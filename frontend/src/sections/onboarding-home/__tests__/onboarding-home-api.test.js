...
vi.mock("src/utils/axios", () => ({
  ...
  endpoints: {
    onboarding: {
      ...
      sendTestTrace: "/accounts/test-trace/",
    },
  },
}));
...
describe("onboarding home API", () => {
  ...
  it("sends a test trace and advances to the trace-ready state", async () => {
    axios.post.mockResolvedValueOnce({
      data: {
        result: {
          activation_state: getActivationStateFixture("observeFirstTraceReady"),
        },
      },
    });

    const next = await sendTestTrace({
      source: "onboarding_home",
      reason: "waiting_for_first_trace",
      projectId: "observe-1",
    });

    expect(axios.post).toHaveBeenCalledWith("/accounts/test-trace/", {
      path: "observe",
      project_id: "observe-1",
      source: "onboarding_home",
      reason: "waiting_for_first_trace",
    });
    // Advances waiting -> review without claiming real activation.
    expect(next.stage).toBe("review_first_trace");
    expect(next.isActivated).toBe(false);
  });

  it("reports a missing test-trace endpoint explicitly", async () => {
    const original = endpoints.onboarding.sendTestTrace;
    endpoints.onboarding.sendTestTrace = null;
    await expect(sendTestTrace()).rejects.toBeInstanceOf(
      OnboardingEndpointUnavailableError,
    );
    endpoints.onboarding.sendTestTrace = original;
  });
});
