export const ACTIVATION_STATE_SCHEMA_VERSION = "activation-state-2026-06-08.v2";
...
const normalizeSignals = (raw = {}) => ({
  ...
  testTraceSupported: Boolean(
    raw.test_trace_supported ?? raw.testTraceSupported,
  ),
...
const normalizeDailyQualitySignal = (raw) => {
  ...
  return {
    id: raw.id,
    path: normalizeProductPath(raw.path ?? raw.primary_path ?? raw.primaryPath),
    type: raw.type,
...
const normalizeValueSignal = (raw) => {
  if (!raw) return null;
  const latencyMs = raw.latency_ms ?? raw.latencyMs ?? null;
  const cost = raw.cost ?? null;
  const totalTokens = raw.total_tokens ?? raw.totalTokens ?? null;
...
export const normalizeActivationState = (raw) => {
  ...
  const state = {
    ...
    activatedVia: raw.activated_via ?? raw.activatedVia ?? null,
    ...
    valueSignal: normalizeValueSignal(raw.value_signal ?? raw.valueSignal),
