...
import { useSendTestTrace } from "./hooks/useSendTestTrace";
import {
...
import PathSamplePreviewPanel from "./components/PathSamplePreviewPanel";
import {
  getPathSampleFixture,
  isPathSampleFirstStage,
} from "./components/path-sample-fixtures";
import {
...
export default function OnboardingHomeView() {
  ...
  const testTraceActions = useSendTestTrace();
  ...
  const handleDailyActionClick = (action, dailyAction) => {
    ...
    trackOnboardingHomeEvent(OnboardingHomeEvents.dailyQualityActionOpened, {
      ...dailyTrackContext,
      action_path: dailyAction?.path || action?.analytics?.targetPath,
  ...
  const handleDailySignalReview = (signal, dailyAction) => {
    ...
    recordActivationEvent.mutate?.({
      ...
      primaryPath: signal.path || renderedState.primaryPath,
      stage: renderedState.stage,
  ...
  const handleDailyActionResolve = (dailyAction, resolution) => {
    ...
    recordActivationEvent.mutate?.(
      {
        eventName,
        primaryPath: dailyAction.path || renderedState.primaryPath,
        stage: renderedState.stage,
        source: "daily_quality_home",
        ...activationEmailContextFor(dailyTrackContext),
  ...
  // Sends a clearly-labelled TEST trace so a waiting first-run user gets a
  // deterministic first signal. The backend advances waiting -> trace-ready but
  // does not mark real activation; we just refetch so the panel moves forward.
  const handleSendTestTrace = async () => {
    if (!renderedState) return;
    trackOnboardingHomeEvent(OnboardingHomeEvents.testTraceSendClicked, {
      ...trackContext,
      action_path: "observe",
      is_sample: false,
    });
    try {
      await testTraceActions.sendTestTrace.mutateAsync({
        path: "observe",
        source: "onboarding_home",
        reason: renderedState.stage,
        projectId: renderedState.signals?.firstObserveId,
        ...activationEmailContextFor(trackContext),
      });
      refetch?.();
    } catch (testTraceError) {
      trackOnboardingHomeEvent(OnboardingHomeEvents.testTraceSendFailed, {
        ...trackContext,
        action_path: "observe",
        is_sample: false,
        reason: testTraceError?.message || "unknown_error",
      });
    }
  };
  ...
  // One primary action is the rule. The error/feature-disabled fallback assigns
  // the same action id to both slots, so compare by id (the two slots are
  // normalized into distinct object references) and treat a missing fallback or
  // a matching id as "no distinct alternate" to avoid rendering twin cards.
  const hasDistinctFallbackAction = Boolean(
    renderedState.fallbackAction &&
      renderedState.fallbackAction.id !== renderedState.recommendedAction?.id,
  );

  // feature_disabled is reached when the activation API is off or errored. We
  // still owe the user a clear way forward, so frame the single CTA below with
  // a helpful lead-in. This state never carries a primaryPath (both the API
  // error fallback and the disabled state leave it null), so the copy stays
  // path-neutral and points at the one real next step rendered below.
  const featureDisabledLeadIn =
    renderedState.stage === "feature_disabled"
      ? {
          title: "Pick up product setup where you left off",
          description:
            "Live setup status is unavailable for this workspace right now, but the next step below still works. Open it to keep going.",
        }
      : null;
  ...
  const firstLoopCompletePanel = ["activated", "daily_review"].includes(
    ...
    ? dailyQualityPanel || (
        <FirstLoopCompletePanel
          ...
          activatedVia={renderedState.activatedVia}
          valueSignal={renderedState.valueSignal}
        />
      )
    : null;
      ...
      <>
        ...
        {[
          ...
          <WaitingForSignalPanel
            ...
            stage={renderedState.stage}
            canSendTestTrace={Boolean(
              renderedState.signals?.testTraceSupported,
            )}
            onSendTestTrace={handleSendTestTrace}
            isSendingTestTrace={mutationPending(testTraceActions.sendTestTrace)}
          />
        ) : null}
  ...
  const pathSampleFixture =
    renderedState.primaryPath !== "observe" &&
    isPathSampleFirstStage(renderedState.primaryPath, renderedState.stage)
      ? getPathSampleFixture(renderedState.primaryPath, sampleProject)
      : null;
  const pathCurrentStep =
    renderedState.primaryPath !== "observe"
      ? journeyCurrentStep(
          renderedState.journeyPlan ||
      ...
      : null;
  const pathRealSetupHref = hrefWithJourneyGuide(
    observePanelProps.action?.href,
    pathCurrentStep,
  );
  const pathSamplePreviewPanel =
    pathFocusPanel && pathSampleFixture ? (
      <PathSamplePreviewPanel
        fixture={pathSampleFixture}
        primaryPath={renderedState.primaryPath}
        realSetupHref={pathRealSetupHref}
      />
    ) : null;
  ...
  return (
    ...
    >
      <Stack spacing={3} sx={{ maxWidth: 1180, mx: "auto" }}>
        ...
        {featureDisabledLeadIn ? (
          <Alert
            severity="info"
            data-testid="feature-disabled-lead-in"
            sx={{ borderRadius: 1 }}
          >
            <Typography variant="subtitle2" component="div">
              {featureDisabledLeadIn.title}
            </Typography>
            <Typography variant="body2" sx={{ mt: 0.25 }}>
              {featureDisabledLeadIn.description}
            </Typography>
          </Alert>
        ) : null}
        ...
        {quickStartMismatchAction ? (
          ...
          <Box
            sx={{
              display: "grid",
              gridTemplateColumns: hasDistinctFallbackAction
                ? { xs: "1fr", md: "minmax(0, 2fr) 1fr" }
                : "1fr",
              gap: 2,
              alignItems: "stretch",
            }}
