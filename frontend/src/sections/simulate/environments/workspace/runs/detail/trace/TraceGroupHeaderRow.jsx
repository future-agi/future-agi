...
import { Box, Stack, Typography, TableCell, TableRow, Checkbox } from "@mui/material";
...
const DESC_KEYS = ["callDetails", "persona", "scenario"];
...
export default function TraceGroupHeaderRow({ group, collapsed, onToggle, show, showEvals, evals, selected, onToggleGroup }) {
  const cellSx = {
    bgcolor: "transparent",
    borderBottom: "1px solid", borderColor: "divider", borderLeft: "none",
      ...
      <TableCell sx={numCellSx}>
        {value == null ? (
          ...
          <Stack direction="row" alignItems="center" spacing={0.5}>
            {bad && <Iconify icon="solar:danger-triangle-bold" width={13} sx={{ color: BUILD_TONES.red, flexShrink: 0 }} />}
            <Typography sx={{
              typography: "s2", fontWeight: "fontWeightBold", fontVariantNumeric: "tabular-nums",
  ...
  const label = (
    <Stack direction="row" alignItems="center" spacing={1.25} sx={{ minWidth: 0 }}>
      <Iconify
        ...
        width={13} sx={{ color: "text.subtitle", flexShrink: 0 }}
      />
      <Typography noWrap sx={{ typography: "s2", fontWeight: "fontWeightBold", color: "text.primary" }}>
        {group.label}
      </Typography>
  ...
  return (
    <TableRow onClick={onToggle}>
      <TableCell
        sx={{
          width: 48, pl: 1.25, pr: 0, py: 0, verticalAlign: "middle",
      ...
      >
        <Checkbox
          size="small" checked={allOn} indeterminate={someOn}
          onChange={() => onToggleGroup?.(group.rows)} sx={neutralCheckboxSx}
        />
      </TableCell>
      {descColumns.length === 0 ? (
        <TableCell sx={{ ...cellSx, pl: 2, overflow: "hidden" }}>{label}</TableCell>
      ) : descColumns.map((key, i) => (
        <TableCell key={key} sx={{ ...cellSx, pl: i === 0 ? 2 : 1.5, overflow: "hidden" }}>
          {i === 0 ? label : (
      ...
      {showEvals && evals.map((e) => {
        ...
        return (
          <TableCell key={`eval-${e.id}`} sx={{ ...numCellSx, p: 0, position: "relative" }}>
            <Box sx={{
              position: "absolute", inset: 0, display: "flex", alignItems: "center",
              px: 2, py: 1.5, bgcolor: interpolateColorBasedOnScore(meanScore, 1), color: "text.primary",
            }}>
              <Typography sx={{ typography: "s2", fontWeight: "fontWeightSemiBold", fontVariantNumeric: "tabular-nums", color: "text.primary" }}>
                {rate}%
              </Typography>
            </Box>
          </TableCell>
