...
describe("annotation selector read lifecycle", () => {
  ...
  it("aborts on unmount and ignores a late response to the destroyed grid", async () => {
    ...
    const { result, unmount } = renderHook(() =>
      useExactSelectorDataSource(config),
    );
    const params = gridParams();
    params.api.isDestroyed = vi.fn(() => false);
    let pending;
    act(() => {
      pending = result.current.dataSource.getRows(params);
    });
    ...
    await act(async () => {
      resolve(
        response({ rows: [{ id: "stale" }], hasMore: false, cursor: null }),
      );
      await pending;
    });
    expect(params.success).not.toHaveBeenCalled();
  ...
  it("settles the old grid read when the query changes and loads a fresh page", async () => {
    ...
    const request = vi
      ...
      .mockResolvedValue(
        response({ rows: [{ id: "new" }], hasMore: false, cursor: null }),
      );
    const config = options(request);
    const { result, rerender } = renderHook(
      ({ signature }) =>
        useExactSelectorDataSource({ ...config, querySignature: signature }),
      { initialProps: { signature: "old" } },
    );
    const oldParams = gridParams();
    let pending;
    act(() => {
      pending = result.current.dataSource.getRows(oldParams);
    });
    ...
    await act(async () => {
      await result.current.dataSource.getRows(fresh);
    });
    expect(fresh.success).toHaveBeenCalledWith({
      rowData: [{ id: "new" }],
      rowCount: 1,
    });
    await act(async () => {
      resolve(
        response({ rows: [{ id: "old" }], hasMore: false, cursor: null }),
      );
    });
