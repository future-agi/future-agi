...
/*
  Trial presets identical to the SelectionBar / TrialsPicker so the
  vocabulary is one thing. Kept here so this dialog stays a leaf
  component with no cross-imports.
*/
const TRIAL_PRESETS = [1, 3, 5, 8];

/**
 * Run configuration dialog — opens when the user clicks the
 * "Run simulation" button in the environment header.
 *
 * Shows:
 *   - How many scenarios will run (read-only)
 *   - Trials-per-scenario picker (1/3/5/8/custom)
 *   - Total runs + rough time estimate
 *   - Cancel + Run buttons
 *
 * Modal so the "how many times?" question is unmissable — the inline
 * TrialsPicker pill in the header is faster once you know it exists,
 * this dialog is what surfaces the choice for first-time users.
 */
