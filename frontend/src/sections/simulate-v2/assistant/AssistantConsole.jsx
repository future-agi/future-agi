...
export default function StudioConsole({ turns, running, chips, onSend, onChip, preComposer, frozen = false, frozenReason }) {
  ...
  */
  ...
  /* Follow the tail only while the reader is at the tail. A run appends every couple of seconds,
     and scrolling to the end on each append made it impossible to read anything further up: you
     scroll, the next event arrives, and you are back at the bottom. */
...
function Step({ step }) {
  ...
  if (step.kind === "think" || step.kind === "note") {
    /* The harness writes markdown: headings, bold, lists and tables. Rendered as plain text it
       arrived as asterisks and pipes, so it goes through the same renderer the rest of the app
       uses. The wrapper keeps Vel's type scale and strips the block margins markdown brings. */
    return (
      <Typography
        component="div"
        sx={{
          ...
          "& > :first-of-type": { mt: 0 },
          "& > :last-child": { mb: 0 },
          "& p, & ul, & ol, & table": { my: 0.75 },
          "& ul, & ol": { pl: 2.5 },
          "& code": { fontSize: "0.85em" },
          "& table": { borderCollapse: "collapse", width: "100%" },
          "& th, & td": {
            border: "1px solid", borderColor: "divider", px: 1, py: 0.5, textAlign: "left",
          },
        }}
      >
