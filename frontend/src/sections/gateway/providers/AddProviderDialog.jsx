...
const AddProviderDialog = ({ open, onClose, gatewayId, provider }) => {
  ...
  useEffect(() => {
    if (open && isEditMode && provider) {
      ...
      setApiKey("");
      setApiFormat(
        c.api_format ?? c.apiFormat ?? provider.api_format ?? "openai",
      );
      setDisplayName(
        provider.display_name ||
          PROVIDER_PRESETS[provider.name || provider.provider_name]?.label ||
          "",
      );
      setModels(
      ...
      setTimeoutVal(
        c.default_timeout ??
          c.defaultTimeout ??
          provider.default_timeout_seconds ??
          "",
      );
      setMaxConcurrent(
  ...
  const handleSave = () => {
    ...
    if (isEditMode) {
      if (isAwsAuth) {
        ...
        if (timeoutVal)
          payload.default_timeout_seconds = parseInt(timeoutVal, 10);
      ...
      const payload = {
        ...
        api_format: apiFormat,
        models_list: models,
        default_timeout_seconds: timeoutVal
          ? parseInt(timeoutVal, 10)
          : DEFAULT_TIMEOUT_SECONDS,
        max_concurrent: maxConcurrent
          ? parseInt(maxConcurrent, 10)
  ...
  const buildGatewayConfig = () => {
    ...
    if (timeoutVal) config.default_timeout = parseInt(timeoutVal, 10);
  ...
  const handleSaveSuccess = (action, resp) => {
    ...
    if (
      resp?.result?.gateway_synced === false ||
      resp?.gateway_synced === false
    ) {
      enqueueSnackbar(
