// Actors (ported from the designer actors fixture).
//
// Not the person the agent is serving — that is a persona, and it lives on the
// scenario rows. An actor is a third party in the world with a goal of its own,
// and that goal is not the task. You are trying to book a cab; your colleague
// says let's get pizza instead — your colleague is the actor. So actors belong
// to transition dynamics rather than to the task: the persona states the goal,
// the actor pulls against it.
//
// Two Phase-3 surfaces read this: the Contract tab's actors panel renders the
// full record per cast member (goal, pressure kind, entry, traits, versions),
// and CapabilityGraph reads `name`. `castFor(env)` seeds a default cast by
// modality fit; the entry order matters because it slices the first three.
...
// What kind of pressure the actor applies — its role in the world. Drives what
// a failure looks like. Colours read from BUILD_TONES.
export const PRESSURE_KINDS = [
  { id: "competing", label: "Competing goal", color: BUILD_TONES.orange, blurb: "Wants something else to happen instead." },
  { id: "authority", label: "Authority", color: BUILD_TONES.accent, blurb: "Outranks the user, or claims to." },
  { id: "gatekeeper", label: "Gatekeeper", color: BUILD_TONES.blue, blurb: "Can block the task, and has its own criteria." },
  { id: "extraction", label: "Extraction", color: BUILD_TONES.red, blurb: "Wants something it is not entitled to." },
...
export const ACTOR_LIBRARY = [
  {
    ...
    version: "v2",
    versions: [
      { label: "v2", note: "Now repeats the counter-proposal after the agent ignores it once." },
      { label: "v1", note: "First draft — the cab-versus-pizza case." },
    ],
    usedBy: 7,
    owner: "you",
  },
  {
    ...
    version: "v2",
    versions: [
      { label: "v2", note: "Now refuses the handover if no summary is given." },
      { label: "v1", note: "First draft." },
    ],
    usedBy: 5,
    owner: "you",
  },
  {
    ...
    version: "v1",
    versions: [{ label: "v1", note: "Approves 60% of the time." }],
    usedBy: 3,
    owner: "system",
  },
  {
    ...
    version: "v1",
    versions: [{ label: "v1", note: "First draft." }],
    usedBy: 2,
    owner: "you",
  },
  {
