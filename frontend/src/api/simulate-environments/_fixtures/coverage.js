// The coverage matrix (verbatim port of the designer coverage mock).
//
// "32 scenarios" says nothing about whether the hard cases are covered. This
// cross-tabulates what is actually in the suite so an empty cell is visible — the
// point is the gaps, not the total.
//
...
// harder than a routine tool call. The UI says so rather than presenting it as a
...
// rows × two axes → { cols, rows, at(r,c), total, empty }
...
// PRD-aligned six-axis coverage (Story 9). The block above is the older
// "mode × difficulty" cut kept for legacy callers; below is the six-axis
// framework the PRD mandates — every scenario is a coordinate over
// T · W · D · X · I · O, with rare-catastrophic O cells forced (AC-9.9).
//
// A scenario's coordinate is derived from its already-existing fields
// (name/title/task/situation/persona/turns/mode) rather than a new stored
// shape. The derivations are deliberately simple so it is clear what is
// being inferred.

// Axis T — Task intent: the 12 closed operations (§9.2). Derived from
// scenario name/title/task keywords.
export const T_OPS = [
...
export const opOf = (row) => {
  ...
  // Most-specific first (auth/handoff/cancel are stronger signals than a
  // generic retrieve match).
  const order = ["cancel", "authenticate", "handoff", "execute", "create", "update",
    "configure", "compare", "explain", "diagnose", "navigate", "retrieve"];
  for (const op of order) {
    if (T_KEYWORDS[op].some((kw) => hay.includes(kw))) return op;
  }
  return "retrieve"; // conservative default; "look up" is the null intent
};

// Axis W — Counterparty. Life-stage is the primary read (what the persona
// label shows).
...
// Axis D — Disposition. Primary read: valence.
...
// Axis X — Interface & environment. Primary read: medium fidelity, seeded
...
export const fidelityOf = (row, env) => {
  ...
  if (surface.includes("voice")) return "noisy"; // voice defaults noisier than chat
  return "clean";
};

// Axis I — Interaction dynamics. Primary read: turn structure from turns.
...
// Axis O — Adversarial/safety overlay. Primary read: overlay type. The
// forced-cells list below materialises AC-9.9.
...
export const overlayOf = (row) => {
  ...
  // Adversarial failure-mode is a strong signal even if no keyword hit.
...
// Rare-catastrophic O overlays the PRD (AC-9.9) says must be present even if
// pairwise sampling skips them.
export const FORCED_O_CELLS = [
  "destructive", "vulnerable", "emergency", "pii", "prompt-injection",
];

// Per-axis coverage: how many of the axis's possible levels are actually hit by
// the current suite, and which are missing.
...
// Forced rare-catastrophic cells (AC-9.9): each critical overlay with whether
// at least one scenario carries it.
...
// The pairwise matrix for a specific axis pair — for the heatmap the
// CoverageMatrix renders.
