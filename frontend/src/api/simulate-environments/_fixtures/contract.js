/**
 * The agent contract, and the sub-tasks that make a scenario runnable.
 *
 * A contract is what the agent *is* — the tools it really has, the services it
 * depends on, the rules it is told and graded against, and what it is actually
 * for. A flat list of tools and rules does not say where each came from and who
 * uses it; a contract does. Everything here derives from the environment, so
 * every environment gets it rather than one hand-written example.
 *
 * Ported from the prototype's contract mock. The scenario-proving half
 * (`validate`, `VALIDATION_CHECKS`, `scenarioFolder` and their helpers) is left
 * to the scenario fixtures; only `contractFor`, `subTasksFor` and
 * `describeUseCase` live here. No hex, no twin branches.
 */
 ...
 *
 * A scenario is one main task; every real task is a small sequence of moves —
 * recognise what is being asked, do the lookups, take the action, close the
 * loop. The runner watches for each of these to land. Kept short (four to six)
 * and deterministic (seeded on the scenario id) so two people looking at the
 * same scenario see the same breakdown, and so a scenario that emphasises
 * refusal reads differently to one that emphasises tool use.
 */
export const subTasksFor = (row, env) => {
  ...
  const refTools =
    kind === "rule" || kind === "adversarial"
      ? []
      : Array.from(
          { length: Math.min(2 + (i % 2), Math.max(tools.length, 1)) },
          (_, n) => tools[(i + n) % Math.max(tools.length, 1)]?.name,
        ).filter(Boolean);
 ...
 *
 * Scenarios don't carry an explicit use-case field, but the id encodes the kind
 * (core / rule / trap / adversarial / edge) and the tools used in the reference
 * solution tell you *which* of them a core scenario is exercising.
 */
export const describeUseCase = (row) => {
  ...
  return {
    ...
    kindLabel,
    detail,
    // Stable id: kind alone for non-core, kind::tool for core.
