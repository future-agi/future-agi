...
// Single read point for the detail response's eval list. Anything that is not
// a list of evals — including the older array-shaped payload — reads as empty.
export const evalsOf = (evalScores) =>
  Array.isArray(evalScores?.evals) ? evalScores.evals : [];
...
export function spanOwnEvalRows(entry) {
  ...
  const rows = [];
  for (const ev of evalsOf(entry?.eval_scores)) {
