...
func TestCreateSpeech_Success(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write(wrapPCMInWAV([]byte{0x01, 0x02}, defaultSampleRate))
	}))
	defer server.Close()
	...
	if !bytes.Equal(data[:4], []byte("RIFF")) {
		t.Errorf("stream header = %q, want RIFF", data[:4])
	}
}
...
func TestCreateSpeech_DefaultsVoiceWhenEmpty(t *testing.T) {
	...
	if gotVoice != "quinn" {
		t.Errorf("voice_id = %q, want default %q", gotVoice, "quinn")
	}
}

func TestCreateSpeech_DefaultsToProVoiceForProModel(t *testing.T) {
	var gotVoice string
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body := make(map[string]any)
		_ = json.NewDecoder(r.Body).Decode(&body)
	...
	if gotVoice != "rhea" {
		t.Errorf("voice_id = %q, want default %q", gotVoice, "rhea")
	}
}

func TestCreateSpeech_WrapsRawPCMAsWAV(t *testing.T) {
	...
	data, err := io.ReadAll(stream)
	if err != nil {
		t.Fatalf("reading WAV stream: %v", err)
	}
	if contentType != "audio/wav" {
		t.Errorf("contentType = %q, want audio/wav", contentType)
	}
	if !bytes.Equal(data[:4], []byte("RIFF")) {
		t.Fatalf("header = %q, want RIFF", data[:4])
	}
	if got := int(binary.LittleEndian.Uint32(data[24:28])); got != defaultSampleRate {
		t.Errorf("sample rate = %d, want %d", got, defaultSampleRate)
	}
	if !bytes.Equal(data[44:], rawPCM) {
		t.Errorf("PCM payload = %v, want %v", data[44:], rawPCM)
	}
}

func TestCreateSpeech_RejectsEmptyAudio(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))
	defer server.Close()
	...
	if len(p.semaphore) != 0 {
		t.Errorf("semaphore len = %d after error, want 0", len(p.semaphore))
	}
}

func TestCreateSpeech_RejectsEmptyNonWAVAudio(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))
	defer server.Close()
...
func TestCreateSpeech_RejectsUnsupportedFormat(t *testing.T) {
	p := newTestProvider(t, "https://api.smallest.ai/waves/v1")
	defer func() { _ = p.Close() }()
...
func TestCreateSpeech_RejectsHeaderOnlyWAV(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write(wavHeader(0, defaultSampleRate))
	}))
	defer server.Close()
	...
	if err == nil {
		t.Fatal("expected empty WAV error, got nil")
	}
}
...
func TestCreateSpeech_ReleasesSemaphoreOnClose(t *testing.T) {
	...
	case <-time.After(time.Second):
		t.Fatal("second stream close blocked")
	}
}
...
func TestCreateTranscription_Success(t *testing.T) {
	...
	if gotQuery != "language=hi&model=pulse" {
		t.Errorf("query = %q, want language=hi&model=pulse", gotQuery)
	}
...
func TestCreateTranscription_DefaultsLanguageWhenEmpty(t *testing.T) {
	...
	if gotQuery != "language=en&model=pulse-pro" {
		t.Errorf("query = %q, want language=en&model=pulse-pro (default language)", gotQuery)
	}
}

func TestCreateTranscription_EscapesQueryParameters(t *testing.T) {
	var gotLanguage string
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotLanguage = r.URL.Query().Get("language")
		w.WriteHeader(http.StatusOK)
		_, _ = w.Write([]byte(`{"status":"success","transcription":"ok"}`))
	}))
	defer server.Close()
	...
	_, err := p.CreateTranscription(context.Background(), &models.TranscriptionRequest{
		Model:    "pulse",
		FileData: []byte("audio"),
		Language: "multi eu&debug=true",
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if gotLanguage != "multi eu&debug=true" {
		t.Errorf("language = %q, want escaped original value", gotLanguage)
	}
}
