...
REPO = HERE.parents[2]
QUALIFIER_DIR = REPO / "futureagi/scripts/th7247_current_select_qualifier"
...
class MaterializerFixture(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.temporary = tempfile.TemporaryDirectory(prefix="th7247-materializer-test-")
        ...
        self.run_id = "th7247-dev-test-001"
    ...
    def _valid_config() -> dict[str, object]:
        return {
            ...
            "image": {
                "derived": (
                    "us-docker.pkg.dev/futureagi-dev/th7247-qualifier-dev/"
...
kind: Job
metadata:
  name: th7247-current-select-qualifier-__QUALIFIER_SHARD__
spec:
...
class PackagingSafetyTests(unittest.TestCase):
    def test_manifest_bound_runtime_deletion_only_removes_exact_regular_file(self) -> None:
        with tempfile.TemporaryDirectory(prefix="th7247-delete-test-") as raw:
    ...
    def test_runtime_deletion_refuses_symlink_parent_and_unsafe_path(self) -> None:
        with tempfile.TemporaryDirectory(prefix="th7247-delete-link-test-") as raw:
            root = Path(raw) / "backend"
...
class SSHMaterializerFixture(MaterializerFixture):
    def setUp(self) -> None:
        ...
        self.env_file = (
            "/home/ubuntu/th7247-qualifier-dev/"
            "th7247-qualifier-dev-readonly.env"
        )
    ...
    def _host_config(self) -> dict[str, object]:
        return {
            ...
            "host": {
                ...
                "remote_workdir": "/home/ubuntu/th7247-qualifier-dev",
            },
            "runtime": {
                "container_runtime": "docker",
                "container_network": "th7247-qualifier-dev-readonly",
    ...
    def test_ssh_plan_is_source_bound_hardened_and_does_not_assume_kubernetes(self) -> None:
        ...
        for value in (
            ...
            "--network=th7247-qualifier-dev-readonly",
