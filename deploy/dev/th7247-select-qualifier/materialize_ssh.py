#!/usr/bin/env python3
"""Materialize an inert TH-7247 qualifier plan for one exact DEV SSH host.
...
FORMAT = "futureagi.th7247-select-qualifier-dev-ssh"
VERSION = 1
...
def load_config(path: Path) -> HostConfig:
    ...
    if not {"th7247", "qualifier", "dev"}.issubset(work_tokens):
    ...
    if (
        ...
        or not {"th7247", "qualifier", "dev", "readonly"}.issubset(
            network_tokens
        )
    ):
    ...
    if not {"th7247", "qualifier", "dev", "readonly"}.issubset(env_tokens):
