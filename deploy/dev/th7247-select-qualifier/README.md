# TH-7247 DEV SELECT-only qualification materializer
...
- an isolated `th7247_catalog_dev_*` database and sorted workspace UUID
...
`futureagi/scripts/th7247_current_select_qualifier/README.md`, build the derived
...
```sh
python3 deploy/dev/th7247-select-qualifier/materialize.py \
  --bundle /private/reviewed-th7247-bundle \
  --config /private/th7247-dev-qualifier.yaml \
  ...
  --shard whatfix \
  --run-id th7247-dev-qualification-001 \
...
python3 deploy/dev/th7247-select-qualifier/materialize.py \
  ...
  --output-directory /private/th7247-whatfix-manifests
```
...
python3 deploy/dev/th7247-select-qualifier/materialize.py \
  ...
  --output-directory /private/th7247-mudflap-manifests
```
...
PYTHONDONTWRITEBYTECODE=1 python3 -m unittest discover \
  -s deploy/dev/th7247-select-qualifier -p 'test_*.py' -v
PYTHONDONTWRITEBYTECODE=1 python3 -m py_compile \
  deploy/dev/th7247-select-qualifier/*.py \
  futureagi/scripts/th7247_current_select_qualifier/*.py
```
...
```sh
python3 /home/ubuntu/th7247-dev-qualifier-current-0816h/bundle/phase-runner-0816h.py \
  --contract \
  /home/ubuntu/th7247-dev-qualifier-current-0816h/bundle/kartik-smoke-0816h-run-contract.json
```
...
PYTHONDONTWRITEBYTECODE=1 python3 \
  deploy/dev/th7247-select-qualifier/test_phase_runner.py
```
...
`th7247-qualifier-dev-readonly` container network; it does not assume the
...
futureagi/.venv/bin/python \
  deploy/dev/th7247-select-qualifier/materialize_ssh.py \
  --bundle /private/reviewed-th7247-bundle \
  --config /private/th7247-dev-ssh.yaml \
