#!/usr/bin/env python3
"""Materialize one source-bound CATALOG qualifier Job for an exact DEV target.
...
FORMAT = "futureagi.select-qualifier-dev"
VERSION = 1
QUALIFIER_SCHEMA = "fi-current-select-only/v2"
QUALIFIER_SHARDS = (
    "whatfix",
    "colektia",
    "mudflap",
...
WORKLOAD_LABEL = "fi-current-select-qualifier"
...
def load_config(path: Path) -> Config:
    ...
    if re.fullmatch(r"fi_catalog_dev_[a-z0-9][a-z0-9_]*", catalog_database) is None:
        raise MaterializationError("catalog database is not an isolated CATALOG DEV name")
...
def _atomic_write_directory(
    ...
    temporary = Path(
        tempfile.mkdtemp(prefix=".fi-materialize-", dir=output_directory.parent)
    )
    try:
        _write_private_file(
