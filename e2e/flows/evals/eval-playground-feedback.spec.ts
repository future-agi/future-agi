...
test('EVAL-E2E-012: add feedback to a logged eval result from the Usage tab', {
  ...
  annotation: flowAnnotation({
    ...
    backendChecks: ['a first-time Add Feedback submission always posts log_id + value + explanation + '
                    + 'action_type to the eval-playground feedback endpoint, independent of the log\'s own source',
  ...
  await test.step('UI: open the log on the Usage tab', async () => {
    ...
    await expect(page.getByRole('button', { name: 'Add Feedback' })).toBeVisible({ timeout: 15_000 });
  });

  await test.step('UI: submit feedback for this result', async () => {
    await page.getByRole('button', { name: 'Add Feedback' }).click();
    // Confirms the drawer opened in "add" (not "edit") mode.
    await expect(page.getByText('Feedbacks for Auto Learning')).toBeVisible({ timeout: 15_000 });
    ...
    await expect(page.getByRole('button', { name: 'Edit Feedback' })).toBeVisible({ timeout: 15_000 });
  ...
  await test.step('UI: the feedback is listed on the Feedback tab', async () => {
    ...
    await expect(page.getByText(improvementNote)).toBeVisible({ timeout: 15_000 });
    ...
    await expect(page.getByText('Log ID')).toBeVisible({ timeout: 10_000 });
  });
