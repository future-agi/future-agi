...
6. map the code's "span_label" parameter to the span's name
7. click Test Evaluation and read the Pass verdict and reason

**Backend state verified:**

- TracingTestMode fetches the real span via getSpansForObserveProject + getTrace, not a mock
- the Test Evaluation button stays disabled until the mapped variable + a loaded row make the tab ready (TracingTestMode's onReadyChange -> EvalDetailPage's isPlaygroundReady); the eval's variable is named so it cannot be filled by TracingTestMode's same-name auto-map
